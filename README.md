...
Everything is rendered from Jinja2 templates. User-facing settings live in `custom-vars.yaml` (repo root); infrastructure defaults live in `core/advanced-vars.yaml`. Secrets (CA password, TSIG keys) are pre-generated by `01-handle-vars.yml` into a git-ignored `core-secrets.yml` before rendering begins.
