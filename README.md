...
```
Root CA  (RSA 4096, ~20 years — baked into root-ca:local Docker image)
    └── Step-CA Intermediate CA  (RSA 4096, ~15 years — signed by root-ca:local)
...
The root CA private key lives at `/opt/stepca/data/secrets/root_ca.key` (mode `0600`, never in any image layer). The signing container `root-ca:local` embeds only the root CA certificate; the key is bind-mounted read-only at runtime. Step-CA serves as the ACME endpoint and signs all runtime leaf certs via its intermediate CA. DNS-01 challenges can be fulfilled via the primary TSIG key (`acme_dns-01`).
